...
export async function createAndSendOffer(
  ...
  const graduateUser = graduate.userId as any;
  ...
  const offer = new Offer({
    ...
    jobId: job._id instanceof mongoose.Types.ObjectId
      ? job._id
      : new mongoose.Types.ObjectId(String(job._id)),
    companyId: company._id instanceof mongoose.Types.ObjectId
      ? company._id
      : new mongoose.Types.ObjectId(String(company._id)),
    graduateId: graduate._id instanceof mongoose.Types.ObjectId
      ? graduate._id
      : new mongoose.Types.ObjectId(String(graduate._id)),
    status: 'pending',
  ...
  const offerMessage = `Congratulations! ${company.companyName} has sent you an offer for the position of ${job.title}. Please download, sign, and upload the offer document to accept.`;
  ...
  if (graduateUser?._id) {
    ...
    const messageContent = `Congratulations! We are pleased to offer you the position of ${job.title} at ${company.companyName}. Please download the attached offer letter, sign it, upload the signed copy here, and then click 'Accept Offer'.`;
    ...
    await Message.create({
      ...
      fileName: `Offer_Letter_${job.title}_${company.companyName}.pdf`,
      read: false,
    });
    ...
    await createNotification({
      ...
      email: {
        subject: `Job Offer: ${job.title} at ${company.companyName}`,
