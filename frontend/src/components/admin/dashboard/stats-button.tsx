...
const StatsButton:React.FC<StatsButtonProps> = ({title, numbers, analysis, icon}) => {
    const Icon = icon;
    ...
    // --- Determine color ---
    const percentColor =
        percentText.startsWith("-") ? "text-red-500" :
        percentText.startsWith("+") ? "text-green-500" :
        "text-inherit";
    ...
    return (
        <div className='bg-white p-[20px] border border-fade rounded-[20px] font-inter min-w-[300px] flex flex-col gap-[20px]'>
            ...
            <p className='text-[16px] font-normal text-[#1C1C1CBF]'>
                {percentText && (
                    <span className={`${percentColor} font-medium`}>
                        {percentText}
                    </span>
                )}{" "}
                {restText}
