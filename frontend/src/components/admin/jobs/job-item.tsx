...
const JobItem:React.FC<JobItem> = ({ title,company,applicants,location,salary,status,views}) => {
  ...
  return (
    <div className='w-full p-[20px] border border-fade rounded-[10px] bg-white gap-[20px] flex font-inter flex-col'>
        <div className='flex items-start gap-[54px]'>
            ...
            <div className={`text-[14px] font-medium px-2.5 py-[5px] rounded-[20px] ${status==="Active" && "bg-[#EFFFE2] text-[#29C500] "} ${status==="Hired" && "bg-[#FFF8ED] text-[#ED6109] "} ${status==="Closed" && "bg-red-100 text-red-500 "}`}>
                {status}
            </div>
        </div>
