...
async function extractTextFromPDF(file: File): Promise<string> {
  try {
    ...
    throw new Error(
      'Failed to extract text from PDF. Please ensure your PDF is not password-protected and is a valid PDF file.'
    );
  }
}
...
function extractName(text: string): { firstName?: string; lastName?: string } {
  ...
  for (const pattern of stopPatterns) {
    ...
    if (match && match.index !== undefined) {
      ...
      const wordsBeforeTitle = beforeTitle
        .split(/\s+/)
        .filter((w) => w.length > 0);
  ...
  const commaPattern =
    /^([A-Z][A-Za-z]+),\s*([A-Z][A-Za-z]+(?:\s+[A-Z][a-z]*)?)/;
...
function extractRoles(text: string): string[] {
  const roleKeywords: Record<string, string> = {
    ...
    'data engineer':
      'data engineer|data science|machine learning|ml|ai|tensorflow|pytorch',
...
function extractExperienceLevel(text: string): string | undefined {
  ...
  if (/senior|sr\.|lead|principal|architect|manager|director/.test(textLower)) {
    return 'senior level';
  }
...
function extractSummary(text: string): string | undefined {
  ...
  if (summaryStartIndex >= 0 && summaryStartIndex < lines.length) {
    ...
    for (
      ...
      i < Math.min(lines.length, summaryStartIndex + 10);
      i++
    ) {
      const line = lines[i];
      ...
      if (
        /^(EXPERIENCE|EDUCATION|SKILLS|PROJECTS|CONTACT|WORK|CERTIFICATIONS)$/i.test(
          line
        )
      ) {
        break;
